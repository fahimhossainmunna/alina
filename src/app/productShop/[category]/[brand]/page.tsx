...
export default function CategoryProductsDirectPage() {
  ...
  useEffect(() => {
    if (categorySlug && CATEGORY_DIRECT_PRODUCTS[categorySlug]) {
      setProducts(CATEGORY_DIRECT_PRODUCTS[categorySlug]);
    }
    setLoading(false);
  }, [categorySlug]);
            ...
            {products.map((product) => (
              <article key={product.id} className="group flex flex-col relative">
                ...
                <div className="relative aspect-[3/4] rounded-[32px] overflow-hidden bg-[#FCF6F2] border border-[#742709]/5 mb-6 transition-all duration-500 group-hover:shadow-[0_25px_50px_rgba(116,39,9,0.06)]">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black/40 via-black/5 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-500 z-30 flex items-center justify-center">
                    <button className="w-full bg-[#FFFCF9] text-[#742709] text-[10px] font-bold uppercase tracking-[0.25em] py-4 rounded-xl hover:bg-[#742709] hover:text-white flex items-center justify-center gap-2 cursor-pointer shadow-md transition-all duration-300">
